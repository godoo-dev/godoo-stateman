...
def test_plan_exit_code_zero_on_all_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.exit_code == 0, f"Expected exit 0 for all-NoOp plan, got {result.exit_code}:\n{result.output}"
...
def test_plan_exit_code_two_on_changes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("godoo_stateman.cli.commands.plan._plan_impl", _mock_impl)
    result = runner.invoke(app, ["plan", config_path])
    assert result.exit_code == 2, f"Expected exit 2 for plan with changes, got {result.exit_code}:\n{result.output}"
...
def test_plan_exit_code_one_on_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("godoo_stateman.cli.commands.plan._plan_impl", _mock_impl)
    result = runner.invoke(app, ["plan", config_path])
    assert result.exit_code == 1, f"Expected exit 1 on exception, got {result.exit_code}:\n{result.output}"
...
def test_help_still_lists_all_five_commands() -> None:
    ...
    for command in ("plan", "apply", "verify", "import", "snapshot"):
        assert command in result.output, f"Command '{command}' missing from --help:\n{result.output}"
...
def test_plan_help_shows_config_and_verbose() -> None:
    ...
    assert result.exit_code == 0
    assert "config" in result.output.lower(), f"'config' not found in plan --help:\n{result.output}"
...
def test_plan_exits_on_missing_config() -> None:
    ...
    assert result.exit_code != 0, f"Expected non-zero exit for missing config, got {result.exit_code}"
...
def test_plan_calls_plan_impl(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert called_with[0][0] == Path(config_path), f"_plan_impl called with wrong config: {called_with[0][0]!r}"
    assert called_with[0][1] is False, f"verbose should be False by default, got {called_with[0][1]!r}"
...
def test_plan_calls_plan_impl_with_verbose(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert called_with, "_plan_impl was never called"
    assert called_with[0][1] is True, f"verbose should be True when --verbose passed, got {called_with[0][1]!r}"
